...
void output_fusion(const std::string &output_dir,
                   const std::string &file_prefix) {
...
void Run_fusion(h_Bins &h_bins, h_Reads &h_reads,
                h_Reads &h_nj_reads, uint32_t numOfBin,
                uint32_t numOfRead, uint32_t numOf_nj_Read,
                int max_gap, const std::string &output_dir,
                const std::string &file_prefix) {
    // read with junctions
    Detect_fusion(h_bins, h_reads, numOfBin, numOfRead, max_gap);
    // read without junctions
    Detect_fusion(h_bins, h_nj_reads, numOfBin, numOf_nj_Read, max_gap);
...
// #define DEBUG
    ...
    // Run_fusion(h_bins, h_reads, h_nj_reads, numOfBin, numOfRead,
    //            numOf_nj_Read, gene_max_gap, output_dir, file_prefix);
}
...
int main(int argc, char **argv) {
    const char* const short_opts = "g:l:x:y:r:o:t:p:m:h";
    ...
    while ((c = getopt_long(argc, argv, short_opts, long_opts, nullptr)) >= 0) {
        switch (c) {
        case 'g': ga.bin_file = (char *)optarg; break;
